...
M.git_branches = function()
	require('telescope.builtin').git_branches({
		attach_mappings = function(_, map)
			...
			map('n', '<c-n>', actions.git_create_branch)
			return true
		end
	})
end

function set_source(content)
	vim.cmd('so ' .. content)
end

local function select_source(prompt_bufnr, map)
	local function set_the_source()
		local content =
		require('telescope.actions.state').get_selected_entry(prompt_bufnr)
		set_source(content.cwd .. "/" .. content.value)
		-- if close then
...
M.search_sources = function()
	require('telescope.builtin').find_files({
		prompt_title = '< Sources >',
		hidden = true,
		no_ignore = true,
		attach_mappings = function(prompt_bufnr, map)
			select_source(prompt_bufnr, map)
			return true
		end
	})
end
...
local function refactor(prompt_bufnr)
	...
	require("telescope.actions").close(prompt_bufnr)
	require("refactoring").close(content.value)
end
